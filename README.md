...
```
[0xC0, 0x81, 0x36, 0x00]   <- cabecera fija de actualización de bloque
...
Se interceptó el tráfico USB entre SignalRGB y el teclado con **USBPcap + Wireshark**, filtrando por transferencias de Interrupción (`URB_INTERRUPT`). Ahí se identificó la cabecera fija `[0xC0, 0x81, 0x36, 0x00]` que precede a cada bloque de color, y la estructura `[ID_LED, R, G, B]` por cada LED dentro del payload de 64 bytes.
